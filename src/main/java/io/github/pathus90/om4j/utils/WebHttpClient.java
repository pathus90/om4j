...
public final class WebHttpClient {
	 ...
	 *             Exception if error in parser
	 */
	public static <T> T executeWithBearAuthentication(String token, String endpoint, Object request, Class<T> clazz) throws Exception {
	...
	private static HttpResponse sendPostRequest(HttpRequest httpRequest) {
		...
		try {
			...
			response = HttpResponse.builder().status(result.code()).data(Objects.requireNonNull(result.body()).string()).build();
		} catch (Exception e) {
