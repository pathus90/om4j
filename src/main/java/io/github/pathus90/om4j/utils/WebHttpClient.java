...
public final class WebHttpClient {
     ...
     * @throws Exception If there's an error in parsing or the request fails
     */
     ...
     * @throws Exception If there's an error in parsing or the request fails
     */
    public static <T> T executePostWithRequestBody(String token, String endpoint, RequestBase requestBody, Class<T> clazz) throws Exception {
     ...
     * @throws Exception If there's an error in parsing or the request fails
     */
    private static <T> T executePostRequest(String endpoint, Headers headers, okhttp3.RequestBody body, Class<T> clazz) throws Exception {
     ...
     *
     * @param token      Access token
     * @param authPrefix Authorization prefix (e.g., "Basic" or "Bearer")
     * @return Headers
     */
    private static Headers createAuthorizationHeaders(String token, String authPrefix) {
        return new Headers.Builder().add(AUTHORIZATION_HEADER.toLowerCase(Locale.ROOT), authPrefix.concat(token)).build();
    }
    ...
    private static okhttp3.RequestBody createJsonRequestBody(RequestBase requestBody) throws JsonProcessingException {
        return okhttp3.RequestBody.create(MediaType.get(JSON_CONTENT_TYPE), JSONSerializer.serializeToJsonString(requestBody));
    }

    /**
     * Creates an HTTP POST request with the specified URL, headers, and request body.
     *
    ...
    /**
     * Handle response errors by throwing an exception if the status is not successful.
     *
     * @param response The HTTP response
     * @throws MobileMoneyException If the response status is not successful
     */
